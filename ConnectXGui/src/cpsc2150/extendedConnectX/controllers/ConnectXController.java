...
public class ConnectXController {
    ...
    /**
     * <p>
     * The array of possible player characters
     * </p>
     */
    private static final char[] playerChar = {'X', 'O', 'A', 'M', 'T', 'E', 'V', 'J', 'S', 'C'};

    /**
     * <p>
     * Allows the board to reset when a win or tie has occurred
     * </p>
     */
    ...
    public void processButtonClick(int col) {
        ...
        screen.setMessage("It is " + playerChar[(playerTurn + 1) % numPlayers] + "'s turn.");
        if (!curGame.checkIfFree(col)) {
            screen.setMessage("The position is full. It is still player " + playerChar[playerTurn % numPlayers] + "'s turn");
            return;
        }
        curGame.placeToken(playerChar[playerTurn % numPlayers], col);
        //this finds the row the token was placed on
        for (int i = curGame.getNumRows() - 1; i >= 0; i--) {
            BoardPosition pos = new BoardPosition(i, col);
            if (curGame.whatsAtPos(pos) != ' ') {
                screen.setMarker(i, col, playerChar[playerTurn % numPlayers]);
                break;
            }
        }
        if (curGame.checkForWin(col)) {
            screen.setMessage("Player " + playerChar[playerTurn % numPlayers] + " wins! Click anywhere to play again!");
            canRestart = true;
        }
        if (curGame.checkTie()) {
            screen.setMessage("The game is a tie! Click anywhere to play again!");
            canRestart = true;
        }
        playerTurn++;
    }
